...
Agents and memory render through `shared/components/ForceGraph` (one d3-force-3d simulation per view, positions kept across polls, new nodes spawn beside a node they link to). Each graph opens with a bloom (`ForceGraph/reveal.ts`: seeds grow in place, then each ring of neighbors sprouts along its link). Hover previews a node's details (`hooks/useHoverDetails`); click focuses it: the camera flies in, its links sweep bright and gain annotations, and unrelated nodes fade. Pages that search render `shared/components/SearchField` into the `PageHeader` trailing slot: on the graphs it highlights and frames matches and flies to a lone match; on `/ghar` it narrows rooms and devices (`features/ghar/house/search.ts`) and sits beside New room. Graphs show `shared/components/GraphPlaceholder` (a constellation in the graph palette) while loading, empty, offline, or failed. `/agents` and the home Agents tile (desktop) draw the agent forest in 3D: top-level threads on an inner shell, sub-agents on outer shells toward their parent (`features/agents/tree.ts`); the tile is the same scene without interaction. Chat sidebar loads durable human↔agent history from Dimaag `GET /threads` and `GET /agents/:id/messages`, then keeps the open thread live via SSE. Dual-lane busy rules and provisional new-chat routing live under `chrome/chatSidebar/` and `store/chat.ts`. Memory/timeline call Yaad via `shared/api` (`yaad` client; paths live in `shared/api/yaad`). `/memory` (titled Yaad) draws Yaad `POST /graph` as a 3D force-directed network (forces in `features/memory/graph.ts`); every non-memory node is labelled. `/chaavi` is the login password manager (search, reveal/copy, create, edit, delete) over `http://chaavi.dadi/v1`; Vaultwarden remains the encrypted store. Browser autofill uses the Bitwarden extension at `https://chaavi.dadi`. Desktop join fetches `GET /ca` from nas.dadi and installs the mesh CA so the extension trusts that HTTPS endpoint.
